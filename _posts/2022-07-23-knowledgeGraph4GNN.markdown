...
'Museum of Modern Art Collection'</a></i>. Through knowledge graph we explored how artists are conneted and how they influensed each other:
<p></p>
...
In this post as data source we will use Wikipedia text data about the same 20 artists that we used in the previous study. Connections between the artists we will investigate through GNN link prediction model.
...
<ul>
<li>Vectors generated by transformers will be used for GNN link prediction model as node features</li>
<li>Based on highly connected vector pairs we will generate additional graph edges.</li>
</ul>
...
<li>We demonstrated how to apply these techniques to find groups of artists that are highly connected or lowly connected.</li>
</ul>

In the future we are planning to experiment with graph mining and Graph Neural Networks by exploring directed graphs. Directed graph will allow us to calculate pairwise similarities not only by cosines but also by symmetry metrics.</p>
